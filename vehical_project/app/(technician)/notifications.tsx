...
export default function TechnicianNotificationsScreen() {
    ...
    const getIcon = (type: string) => {
        switch (type) {
            case 'service': return 'hammer-wrench';
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={[styles.header, { borderBottomColor: colors.border }]}>
                <TouchableOpacity onPress={() => router.canGoBack() ? router.back() : router.replace('/(technician)/(tabs)')} style={styles.backButton}>
...
const styles = StyleSheet.create({
    ...
    card: { flexDirection: 'row', alignItems: 'center', padding: 16, borderRadius: 16, marginBottom: 12, borderWidth: 1, gap: 12 },
    iconContainer: { width: 48, height: 48, borderRadius: 24, alignItems: 'center', justifyContent: 'center', shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 2 },
    cardTitle: { fontSize: 16, fontFamily: 'NotoSans-Bold', flex: 1 },
    cardBody: { fontSize: 13, marginTop: 4, lineHeight: 18 },
