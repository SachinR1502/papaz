...
export default function SecurityScreen() {
    ...
    useEffect(() => {
        ...
        fetchDevices();
        checkBiometricsAvailabilityAndState();
    }, []);

    const checkBiometricsAvailabilityAndState = async () => {
        const compatible = await LocalAuthentication.hasHardwareAsync();
        setIsBiometricsAvailable(compatible);

        if (compatible) {
            const savedBiometrics = await SecureStore.getItemAsync('biometrics_enabled');
            setBiometrics(savedBiometrics === 'true');
        } else {
            setBiometrics(false); // Disable if not compatible
        }
    };
    ...
    const fetchDevices = async () => {
        try {
            ...
            // Mock if fail
            setDevices([
                { _id: '1', name: 'Admin Console', type: 'Web', lastActive: new Date().toISOString(), isCurrent: true, location: 'Secure Server' }
            ]);
        }
    };

    const handleChangePassword = () => {
        Alert.alert('Reset Password', 'Admin password reset requires 2FA verification.');
    };
            ...
            <ScrollView contentContainerStyle={styles.content}>
                <Animated.View style={{ opacity: fadeAnim }}>
                    <View style={[styles.heroSection, { backgroundColor: colors.shieldBg }]}>
                        <MaterialCommunityIcons name="shield-lock" size={60} color="#34C759" />
                        <Text style={[styles.heroTitle, { color: colors.text }]}>System Secure</Text>
                        <Text style={[styles.heroSub, { color: colors.subText }]}>Administrator access is fully protected.</Text>
                    </View>
                    ...
                    <View style={styles.section}>
                        ...
                        <View style={[styles.card, { backgroundColor: colors.card }]}>
                            <TouchableOpacity style={[styles.row, { borderBottomWidth: 1, borderBottomColor: colors.border }]} onPress={handleChangePassword}>
                                ...
                                <Text style={[styles.rowLabel, { color: colors.text }]}>Change Admin Password</Text>
                            ...
                            <View style={[styles.row, { borderBottomWidth: 1, borderBottomColor: colors.border }]}>
                                ...
                                <View style={styles.rowContent}>
                                    ...
                                    <Text style={[styles.rowSub, { color: colors.subText }]}>Required for sensitive actions</Text>
                                </View>
                                <Switch
                                    value={biometrics}
                            ...
                            <View style={styles.row}>
                                ...
                                <View style={styles.rowContent}>
                                    <Text style={[styles.rowLabel, { color: colors.text }]}>2-Factor Auth (Enforced)</Text>
                                    <Text style={[styles.rowSub, { color: colors.subText }]}>Mandatory for all admins</Text>
                                </View>
                                <Switch
                                    value={twoFactor}
                                    onValueChange={() => Alert.alert("Restricted", "2FA cannot be disabled for admins.")}
                                    trackColor={{ false: '#D1D1D6', true: '#34C759' }}
                                    thumbColor="#FFF"
                                    disabled
                                />
                    ...
                    <View style={styles.section}>
                        <Text style={[styles.sectionTitle, { color: colors.sectionTitle }]}>ACTIVE ADMIN SESSIONS</Text>
                        <View style={[styles.card, { backgroundColor: colors.card }]}>
                            {devices.length === 0 ? (
