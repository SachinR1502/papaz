...
// Reusing MediaCaptureSection for now, unless requested to separate that too
import { MediaCaptureSection } from '../customer/MediaCaptureSection';
...
interface TechnicianPartRequestModalProps {
    visible: boolean;
    onClose: () => void;
    onSubmit: (items: PartRequestItem[], notes: string, supplierId: string | null, photos: string[], voiceNote: string | null, vehicleId: string | null, manualVehicleDetails?: VehicleDetails) => Promise<void>;
...
export const TechnicianPartRequestModal: React.FC<TechnicianPartRequestModalProps> = ({
    visible,
    onClose,
    onSubmit,
    prefillItems = [],
    ...
    const [manualVehicleReg, setManualVehicleReg] = useState('');
    ...
    }, [visible, initialNotes, initialPhotos, initialVoiceNote, prefillItems, vehicles, selectedVehicle]);
    ...
    const handleSubmit = async () => {
        ...
        const finalVehicleId = isManualVehicle ? null : selectedVehicle;
        let finalNotes = customNotes;

        let manualVehicleDetails: VehicleDetails | undefined;
        if (isManualVehicle && manualVehicleMake.trim()) {
            manualVehicleDetails = {
                make: manualVehicleMake,
                model: manualVehicleModel,
                registrationNumber: manualVehicleReg
            };
            // Kept for backward compatibility if backend doesn't use vehicleDetails yet
            // finalNotes += `\n[Vehicle: ${manualVehicleMake} ${manualVehicleModel} ${manualVehicleReg ? `(${manualVehicleReg})` : ''}]`;
        }

        await onSubmit(items, finalNotes, selectedSupplier, photos, voiceNote, finalVehicleId, manualVehicleDetails);
        console.log('Part request submitted successfully', items, finalNotes, selectedSupplier, photos, voiceNote, finalVehicleId, manualVehicleDetails);
    };
                    ...
                    >
                        ...
                        <View style={styles.section}>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                                <Text style={[styles.fieldLabel, { marginBottom: 0, color: colors.text }]}>{t('vehicle_info') || 'Vehicle Information'}</Text>
                                <TouchableOpacity
                                    onPress={() => {
                                        setIsManualVehicle(!isManualVehicle);
                                        if (!isManualVehicle) setSelectedVehicle(null);
                                    }}
                                    style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}
                                >
                                    <Ionicons name={isManualVehicle ? "list-outline" : "add-circle-outline"} size={16} color={colors.primary} />
                                    <Text style={{ fontSize: 12, color: colors.primary, fontWeight: '600' }}>
                                        {isManualVehicle ? (t('select_from_list') || 'Select from list') : (t('add_custom_vehicle') || 'Custom Vehicle')}
                                    </Text>
                                </TouchableOpacity>
                            </View>
                            ...
                            {!isManualVehicle ? (
                                            ...
                                            <ScrollView style={{ maxHeight: 150 }} nestedScrollEnabled>
                                                {vehicles.map((v, i) => (
                                                    ...
                                                    >
                                                        ...
                                                        <Text style={{ fontSize: 11, color: colors.icon }}>{v.brand || v.make} {v.model} • {v.registrationNumber}</Text>
                                                    </TouchableOpacity>
                                                ))}
                                                {vehicles.length === 0 && (
                                ...
                                <View style={[styles.formCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                                    ...
                                    <View style={styles.row}>
                                        ...
                                        <TextInput
                                            style={[styles.input, { flex: 1, color: colors.text, borderColor: colors.border, backgroundColor: isDark ? '#2C2C2E' : '#F9F9F9' }]}
                                            placeholder={t('reg_no') || "Reg #"}
                                            placeholderTextColor={colors.icon}
                                            value={manualVehicleReg}
                                            onChangeText={setManualVehicleReg}
                                        />
                                    </View>
                                </View>
                            )}
                        ...
                        <View style={styles.section}>
                            ...
                            <View style={[styles.mediaCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                                <TextInput
                                    ...
                                    placeholder={t('describe_request_placeholder') || "Describe what you need in detail..."}
                                    placeholderTextColor={colors.icon}
                                    value={customNotes}
